...
await check('Logo Maker 2.0 persists reusable brand settings and exports real SVG',async()=>{
 await page.locator('.dpBoardCard').filter({hasText:'LOGO2 TEST'}).click();const root=page.getByTestId('logo-maker-2-root');await root.waitFor();await page.getByLabel('Logo wordmark').fill('PLANET X');await page.getByLabel('Logo tagline').fill('LOUD SOFTWARE');await page.getByLabel('Transparent background').check();await root.getByRole('button',{name:'SQUARE',exact:true}).last().click();await page.getByLabel('Logo size').selectOption('1024');await page.waitForTimeout(150);
