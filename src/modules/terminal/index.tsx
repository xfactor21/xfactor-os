...
export default function TerminalRoom({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    ...
    fit.fit();
    term.writeln('xFactor.OS Terminal — pick a runtime above to boot it.');
      ...
      <div className="roomInner">
        ...
        <div className="optrow" style={{ margin: '0 0 12px' }}>
          ...
          <span
            ...
            title="Two separate reasons: (1) no E2B account configured — no provider key is present in this release. (2) No maintained self-hosted C/C++-to-WASM compiler exists as a real dependency today (the one community demo was pulled from npm in 2021)."
          >
            OTHER (C/C++, needs E2B)
          </span>
          {status === 'booting' && (
