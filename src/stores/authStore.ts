...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  init: () => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      ...
      // Auth/network failures must never strand the app in a loading state.
      set({ session: null, user: null, status: 'signed-out' });
    });
  ...
  signInWithPassword: async (email, password) => {
    if (!supabaseConfigured) return { error: 'Cloud accounts are not configured for this deployment.' };
    try {
      const { error } = await supabase.auth.signInWithPassword({ email, password });
      return { error: error?.message ?? null };
    } catch (error) {
      return { error: error instanceof Error ? error.message : 'Sign-in failed. Check your connection and try again.' };
    }
  },

  signUpWithPassword: async (email, password) => {
    if (!supabaseConfigured) return { error: 'Cloud accounts are not configured for this deployment.', needsConfirmation: false };
    try {
      const { data, error } = await supabase.auth.signUp({ email, password });
      const needsConfirmation = !error && !data.session;
      return { error: error?.message ?? null, needsConfirmation };
    } catch (error) {
      return { error: error instanceof Error ? error.message : 'Account creation failed. Check your connection and try again.', needsConfirmation: false };
    }
  },
  ...
  sendMagicLink: async (email) => {
    ...
    try {
      ...
      return { error: error instanceof Error ? error.message : 'Magic-link request failed. Check your connection and try again.' };
    }
  },

  signOut: async () => {
    if (!supabaseConfigured) return;
    try { await supabase.auth.signOut(); } catch { /* local session UI stays usable */ }
  },
